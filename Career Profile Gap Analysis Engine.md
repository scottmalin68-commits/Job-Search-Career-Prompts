...
Assign each finding one primary Gap Type (`MISSING`, `UNDERDEVELOPED`, `EVIDENCE_GAP`, `QUANTIFICATION_GAP`, `SCOPE_GAP`, `OWNERSHIP_GAP`, `IMPACT_GAP`, `RECENCY_GAP`, `CHRONOLOGY_GAP`, `CONSISTENCY_GAP`, `AMBIGUITY`) and a strict severity rating:
...
Provide the top 10 targeted, non-leading questions the user can answer immediately to yield the highest impact across multiple downstream tools.
