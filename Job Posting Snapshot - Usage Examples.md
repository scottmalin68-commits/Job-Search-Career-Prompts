...
**Key Note:** Reuse prompts (Interview Prep, Hiring Intent Analysis, etc.) are defined in **Section 6** of the main engine prompt. This manual describes when and how to invoke them against saved snapshots. All steps adhere strictly to the tagging and hallucination control rules defined in the main prompt.
...
### Post-Snapshot Reuse Examples

#### Example 4: Interview Preparation
1. Copy snapshot markdown.
2. Use Section 6 "Interview Preparation" prompt from main engine.
...
**Example:** Over 12 snapshots, "SASE" appears 9 times → prioritize it in skills and learning roadmap.
...
- **If you discover a tagging error, re-run the engine on the original source instead of hand-editing the snapshot.**
...
4. **Update resume using exact phrasing from [VERBATIM] bullets — only where it truthfully describes your experience.**  
